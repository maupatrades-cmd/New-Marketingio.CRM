...
export default function ClientContractDetail() {
  ...
  return (
    <div className="space-y-6">
      <Link to="/client/contracts" className="inline-flex items-center gap-1 text-sm text-soft hover:text-white">
        <ChevronLeft size={16} /> Back to contracts
      </Link>

      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="font-display text-2xl text-white">{PACKAGE_LABEL[c.package] ?? c.package ?? 'Contract'}</h1>
          {c.add_on_name && <p className="text-sm text-soft mt-0.5">{c.add_on_name} (add-on)</p>}
          <p className="text-xs text-soft mt-1">Created {new Date(c.created_at).toLocaleDateString('en-ZA')}</p>
        </div>
        <div className="flex gap-2">
          ...
          {pdfUrl && (
            <a href={pdfUrl} target="_blank" rel="noreferrer"
               className="inline-flex items-center gap-1 rounded-lg border border-darkbg-border bg-darkbg-900 hover:border-brandred text-white px-3 py-2 text-xs transition">
      ...
      <section className="rounded-xl border border-darkbg-border bg-darkbg-800/50 p-4">
        {c.client_signed_at ? (
          <p className="text-sm text-emerald-400"><CheckCircle2 size={14} className="inline mr-1" />
            You signed on {new Date(c.client_signed_at).toLocaleString('en-ZA')}</p>
        ) : (
          <p className="text-sm text-amber-400">Awaiting your signature.</p>
        )}
      </section>
      ...
      <section className="rounded-2xl border border-darkbg-border bg-darkbg-800/50 p-6">
        <div className="flex items-center gap-3 mb-4">
          <Shield size={20} className={passed === checks.length ? 'text-emerald-400' : 'text-amber-400'} />
          <div className="flex-1">
            <div className="flex justify-between text-sm mb-1">
              <span className="text-white font-medium">{passed} of {checks.length} verified</span>
            </div>
          ...
          {checks.map((ch, i) => (
            <li key={i} className="flex items-center gap-2 text-sm">
              {ch.pass
                ...
                : <XCircle size={16} className="text-soft shrink-0" />}
              <span className={ch.pass ? 'text-white' : 'text-soft'}>{ch.label}</span>
            </li>
          ))}
        </ul>
      </section>
      ...
      {c.special_conditions && (
        <section className="rounded-xl border border-darkbg-border bg-darkbg-800/50 p-4">
          <p className="text-xs uppercase tracking-widest text-soft mb-2">Special conditions</p>
          <p className="text-sm text-white whitespace-pre-wrap">{c.special_conditions}</p>
        </section>
      )}
      ...
      {pdfUrl && (
        <section className="rounded-xl border border-darkbg-border bg-darkbg-800/50 p-4">
          <p className="text-xs uppercase tracking-widest text-soft mb-3 flex items-center gap-1"><FileText size={12} /> Document</p>
          <iframe src={pdfUrl} title="Contract" className="w-full h-[60vh] rounded border border-darkbg-border" />
        </section>
      )}
    </div>
  );
