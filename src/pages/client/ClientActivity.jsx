...
const TYPE_ICON = { invoice: Receipt, deliverable: Package, contract: FileSignature, report: BarChart3, message: MessageCircle, system: Info };

// Filter chips — coarse category buckets. We match on the notification_type
// prefix (e.g. any invoice_* notification counts as "invoice") so the
// server can keep firing granular types (invoice_issued, invoice_overdue…)
// without breaking the client chips.
const FILTERS = [
      ...
      {rows.length === 0 ? (
        <div className="mio-glow-border rounded-xl border border-white/80 bg-white/85 backdrop-blur-xl shadow-sm p-8">
          <MascotGuide phase="guide" size={80}
            message={filter === 'all'
              ? "Nothing to show yet — activity will appear here as things happen."
              : `Nothing to show under ${activeFilter.label}.`}
            position="inline" />
        </div>
      ) : (
        <ul className="space-y-2">
          {rows.map(n => {
            ...
            const props = n.action_url
              ? { to: n.action_url, onClick: () => !n.is_read && readOne.mutate(n.id) }
