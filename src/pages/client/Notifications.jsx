  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl text-gradient">Notifications</h1>
          <p className="text-sm text-soft mt-1">{unread > 0 ? `${unread} unread` : 'All caught up'}</p>
        </div>
        {unread > 0 && (
              ...
              <li key={n.id}>
                <Wrapper {...wrapperProps}
                         className={`block rounded-xl border p-3 cursor-pointer transition ${
                           n.is_read
                             ? 'border-darkbg-border bg-darkbg-800/30 hover:bg-darkbg-800/60'
                             : 'border-brandred/30 bg-brandred/5 hover:border-brandred/60'
                         }`}>
                  <div className="flex items-start gap-3">
                    <div className={`h-8 w-8 rounded-lg flex items-center justify-center shrink-0 ${
                      n.is_read ? 'bg-darkbg-700 text-soft' : 'bg-brandred/20 text-brandred'
                    }`}>
                      <Icon size={14} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {!n.is_read && <span className="h-1.5 w-1.5 rounded-full bg-brandred" />}
                      </div>
                      {n.body && <p className="text-xs text-soft mt-0.5">{n.body}</p>}
