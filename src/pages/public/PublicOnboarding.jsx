...
export default function PublicOnboarding() {
  ...
  if (dataQ.isLoading) {
    return <div className="min-h-screen grid place-items-center bg-gradient-to-br from-rose-50 via-purple-50 to-sky-50 text-gray-500"><Loader2 size={24} className="animate-spin" /></div>;
  }
  if (dataQ.isError) {
    return (
      <div className="min-h-screen grid place-items-center bg-gradient-to-br from-rose-50 via-purple-50 to-sky-50 px-4">
        <div className="max-w-md rounded-2xl border border-red-200 bg-red-50 p-8 text-center">
          ...
          <h1 className="font-display text-2xl text-[#0B2143] mb-2">Link invalid or expired</h1>
          <p className="text-sm text-red-700">This onboarding link is no longer valid. Please contact Marketing iO for a fresh link.</p>
        </div>
      </div>
    );
  }
  if (submitted || dataQ.data?.already_submitted) {
    return (
      <div className="min-h-screen grid place-items-center bg-gradient-to-br from-rose-50 via-purple-50 to-sky-50 px-4">
        <div className="max-w-md rounded-2xl border border-emerald-200 bg-emerald-50 p-8 text-center">
          <CheckCircle2 size={40} className="mx-auto text-emerald-500 mb-3" />
          <h1 className="font-display text-2xl text-[#0B2143] mb-2">Thank you!</h1>
          <p className="text-sm text-emerald-700">Your onboarding form has been submitted. Our team will be in touch shortly.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-purple-50 to-sky-50 text-[#0B2143]">
      <div className="mx-auto max-w-3xl px-4 py-8 space-y-8">
        <header className="text-center">
          <p className="font-display text-lg text-red-500 font-semibold">Marketing iO</p>
          <h1 className="font-display text-3xl text-[#0B2143] mt-2">Welcome — let's onboard you</h1>
          <p className="text-sm text-gray-500 mt-2">Fill this in to help us launch your project. Your progress saves automatically.</p>
        ...
        <Section title="Discovery — tell us about your business">
          ...
          <div>
            <label className="label-light">Your #1 goal with marketing</label>
            <select className="input-light" value={form.discovery.goal} onChange={e => setDiscovery('goal', e.target.value)}>
          ...
          <div>
            <label className="label-light">Brand assets readiness</label>
            <select className="input-light" value={form.discovery.brand_ready} onChange={e => setDiscovery('brand_ready', e.target.value)}>
              <option value="">— Select —</option>
        ...
        <Section title="Brand assets">
          ...
          <div>
            <label className="label-light">Upload brand files (logos, photos, guidelines)</label>
        ...
        <Section title="Account access">
          <p className="text-xs text-gray-500">So we can post on your behalf.</p>
        ...
        <Section title="Debit order authorisation">
          ...
          <div>
            <label className="label-light">Signature — draw below to authorise</label>
        ...
        <div className="sticky bottom-4 z-10 rounded-xl border border-gray-200 bg-white/90 backdrop-blur shadow-lg p-4">
          {errMsg && <p className="mb-2 text-sm text-red-500">{errMsg}</p>}
          <button onClick={onSubmit} disabled={submitting}
                  className="w-full inline-flex items-center justify-center gap-2 rounded-full bg-red-500 hover:bg-red-600 text-white px-6 py-3 font-semibold transition">
...
function Field({ label, value, onChange, placeholder }) {
  return (
    <div>
      ...
      <input className="input-light" value={value ?? ''} onChange={e => onChange(e.target.value)} placeholder={placeholder} />
    </div>
  );
}
function TextArea({ label, value, onChange }) {
  return (
    <div>
      <label className="label-light">{label}</label>
      <textarea className="input-light min-h-[70px]" value={value ?? ''} onChange={e => onChange(e.target.value)} />
    </div>
  );
}
function FileInput({ accept, label, multiple, onFile }) {
  return (
    <label className="inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white hover:border-red-300 px-3 py-2 text-xs text-gray-600 hover:text-red-500 cursor-pointer transition">
...
function SignaturePad({ value, onChange }) {
  ...
  const move = (e) => { if (!drawing.current) return; e.preventDefault();
    ...
    ctx.lineWidth = 2; ctx.strokeStyle = '#0B2143'; ctx.lineCap = 'round'; ctx.lineTo(x, y); ctx.stroke(); };
  ...
  return (
    <div>
      ...
      {hasStrokes && (
        <button type="button" onClick={clear} className="mt-1 text-xs text-red-500 hover:text-red-600 transition">
          Clear signature
        </button>
      )}
    </div>
...
function SaveBadge({ status, error }) {
  if (status === 'saving') return <p className="mt-2 text-xs text-gray-500"><Loader2 size={10} className="inline mr-1 animate-spin" />Saving…</p>;
  if (status === 'saved')  return <p className="mt-2 text-xs text-emerald-500"><CheckCircle2 size={10} className="inline mr-1" />Saved</p>;
  if (status === 'error')  return <p className="mt-2 text-xs text-red-500">{error || 'Save failed'}</p>;
  return null;
}
