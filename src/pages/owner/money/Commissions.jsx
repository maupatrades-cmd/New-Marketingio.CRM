...
export default function Commissions() {
  ...
  const { data: rows, isLoading } = useQuery({
    queryKey: ['commissions-page', status, user?.id],
    enabled: !!user,
    queryFn: async () => {
      let q = supabase.from('commissions').select('*').order('created_at', { ascending: false }).limit(1000);
      if (status !== 'all') q = q.eq('status', status);
      // Non-owners only see their own commissions
      if (!isOwner) q = q.eq('staff_id', user.id);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <div className="rounded-2xl border border-darkbg-border bg-darkbg-800/50 p-4">
          <p className="text-xs uppercase tracking-widest text-soft">Total pending</p>
          <p className="mt-2 font-display text-2xl text-white">{money(summary.pending)}</p>
        </div>
        <div className="rounded-2xl border border-darkbg-border bg-darkbg-800/50 p-4">
          <p className="text-xs uppercase tracking-widest text-soft">Paid this month</p>
          <p className="mt-2 font-display text-2xl text-emerald-400">{money(summary.paidThisMonth)}</p>
        </div>
        {isOwner && (
          <div className="rounded-2xl border border-darkbg-border bg-darkbg-800/50 p-4">
            <p className="text-xs uppercase tracking-widest text-soft">Top earner</p>
            <p className="mt-2 font-display text-lg text-white">{summary.top}</p>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search staff or client…"
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-darkbg-border bg-darkbg-800/50 p-12 text-center">
          ...
          <p className="mt-3 font-display text-lg text-white">No commissions found</p>
        </div>
      ) : (
        <div className="overflow-x-auto rounded-2xl border border-darkbg-border">
          <table className="w-full text-sm">
            <thead className="bg-darkbg-800/80 text-left text-[11px] uppercase tracking-widest text-soft">
              <tr>
                {isOwner && <th className="px-4 py-3">Staff</th>}
            ...
            <tbody className="divide-y divide-darkbg-border">
              {filtered.map((r) => (
                <tr key={r.id} className="bg-darkbg-800/30 hover:bg-darkbg-800/60">
                  {isOwner && <td className="px-4 py-3 text-white">{r.staff_name}</td>}
                  ...
                  <td className="px-4 py-3 text-right font-semibold text-white">{money(r.commission_amount)}</td>
                  <td className="px-4 py-3 text-soft capitalize">{(r.qualifying_event || '').replace(/_/g, ' ')}</td>
                  <td className="px-4 py-3">
                    <span className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${BADGE[r.status] || BADGE.pending}`}>
                      {(r.status || '').replace('_', ' ')}
                    </span>
                  </td>
                  {isOwner && (
                    <td className="px-4 py-3">
                      <div className="flex justify-end gap-2">
                        {r.status === 'pending' && (
                          <button onClick={() => onMarkPaid(r)} className="rounded-full bg-emerald-500/15 px-2.5 py-1 text-[11px] font-semibold text-emerald-300 hover:bg-emerald-500/25">
                            Mark paid
                          </button>
                        )}
                        {r.status !== 'clawback' && (
