...
export default function Invoices() {
  ...
  const rows = useMemo(() => {
    ...
    return (invoices ?? []).filter((i) =>
      !s || (i.client_name || '').toLowerCase().includes(s) || (i.invoice_number || '').toLowerCase().includes(s));
  }, [invoices, search]);
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-4">
        ...
        {role === 'owner' && (
          <button onClick={onRefreshOverdue} disabled={refreshing} className="inline-flex items-center gap-2 rounded-full border border-darkbg-border bg-darkbg-800/60 px-4 py-2 text-sm text-white transition hover:bg-darkbg-800 disabled:opacity-40">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {STATUSES.map((s) => (
          <button key={s} onClick={() => setStatus(s)} className={`rounded-full border px-3 py-1.5 text-xs capitalize transition ${status === s ? 'border-brandred bg-brandred/15 text-white' : 'border-darkbg-border bg-darkbg-800/60 text-soft hover:text-white'}`}>{s}</button>
        ))}
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search client or number…" className="ml-auto rounded-full border border-darkbg-border bg-darkbg-900 px-4 py-1.5 text-sm text-white placeholder:text-soft" />
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-darkbg-border bg-darkbg-800/50 p-12 text-center">
          ...
          <p className="mt-3 font-display text-lg text-white">No invoices</p>
        </div>
      ) : (
        <div className="overflow-x-auto rounded-2xl border border-darkbg-border">
          <table className="w-full text-sm">
            <thead className="bg-darkbg-800/80 text-left text-[11px] uppercase tracking-widest text-soft">
              <tr>
                <th className="px-4 py-3">Number</th><th className="px-4 py-3">Client</th><th className="px-4 py-3">Type</th>
                <th className="px-4 py-3 text-right">Amount</th><th className="px-4 py-3">Issued</th><th className="px-4 py-3">Due</th>
            ...
            <tbody className="divide-y divide-darkbg-border">
              {rows.map((i) => (
                <tr key={i.id} className="bg-darkbg-800/30">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-2">
                      {isManager && i.status !== 'paid' && i.status !== 'cancelled' && (
                        <button onClick={() => { setPayModal(i); setPayDate(new Date().toISOString().slice(0, 10)); }} className="rounded-full bg-emerald-500/15 px-2.5 py-1 text-[11px] font-semibold text-emerald-300 hover:bg-emerald-500/25">Mark paid</button>
                      )}
                      {isManager && i.status !== 'cancelled' && i.status !== 'paid' && (
                        <button onClick={() => onCancel(i)} className="rounded-full border border-darkbg-border px-2.5 py-1 text-[11px] text-soft hover:text-white">Cancel</button>
                      )}
                    </div>
                  </td>
                </tr>
      ...
      <Modal
        ...
        title={`Mark ${payModal?.invoice_number} paid`}
        footer={<>
