...
function last12Months() {
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    out.push({ key: monthKey(d), label: d.toLocaleDateString('en-ZA', { month: 'short' }) });
  }
  return out;
}
...
export default function Earnings() {
  ...
  const canView = role === 'owner' || targetId === user?.id;
  ...
  const chartData = useMemo(() => {
    const buckets = Object.fromEntries(last12Months().map((m) => [m.key, 0]));
    ...
    return last12Months().map((m) => ({ month: m.label, amount: Math.round(buckets[m.key]) }));
  }, [commissions]);
  ...
  if (!canView) {
    return <div className="card p-8 text-center"><p className="text-soft">You can only view your own earnings.</p></div>;
  }
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
            <Kpi label="This month" value={money(stats.thisMonth)} />
            <Kpi label="Last month" value={money(stats.lastMonth)} />
            <Kpi label="YTD" value={money(stats.ytd)} />
            <Kpi label="Pending" value={money(stats.pending)} accent="text-amber-400" />
            <Kpi label="Paid" value={money(stats.paid)} accent="text-emerald-400" />
          </div>
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData}>
                  ...
                  <XAxis dataKey="month" stroke="#9aa0aa" fontSize={11} tickLine={false} axisLine={false} />
                  <YAxis stroke="#9aa0aa" fontSize={11} tickLine={false} axisLine={false} tickFormatter={(v) => `R${v / 1000}k`} />
                  <Tooltip
              ...
              <div className="overflow-x-auto rounded-2xl border border-darkbg-border">
                <table className="w-full text-sm">
                  <thead className="bg-darkbg-800/80 text-left text-[11px] uppercase tracking-widest text-soft">
                    <tr><th className="px-4 py-3">Type</th><th className="px-4 py-3">Client</th><th className="px-4 py-3">Event</th><th className="px-4 py-3 text-right">Amount</th><th className="px-4 py-3">Status</th></tr>
                  </thead>
...
function Kpi({ label, value, accent }) {
  return (
    <div className="rounded-2xl border border-darkbg-border bg-darkbg-800/50 p-4">
      ...
      <p className={`mt-2 font-display text-xl ${accent || 'text-white'}`}>{value}</p>
    </div>
  );
}
